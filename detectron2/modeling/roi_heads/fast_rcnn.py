...
class FastRCNNOutputLayers(nn.Module):
    ...
    def losses(self, predictions, proposals):
        ...
        losses = {
            "loss_cls": self.cls_loss(scores, gt_classes),  # cross_entropy(scores, gt_classes, reduction="mean"),
    ...
    def cls_loss(self, scores, gt_classes):
        fg_inds = gt_classes != self.num_classes
        bg_inds = gt_classes == self.num_classes
        fg_cls_loss = cross_entropy(scores[fg_inds], gt_classes[fg_inds], reduction="sum") / scores.shape[0]
        bg_cls_loss = cross_entropy(scores[bg_inds], gt_classes[bg_inds], reduction="sum") / scores.shape[0]

        storage = get_event_storage()

        total_loss = fg_cls_loss + bg_cls_loss

        storage.put_scalar(f"fast_rcnn/loss_fg_cls", fg_cls_loss)
        storage.put_scalar(f"fast_rcnn/loss_bg_cls", bg_cls_loss)
