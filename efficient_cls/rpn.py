...
class RPN(nn.Module):
    ...
    def losses(
        ...
        if weights is not None:
            localization_loss = localization_loss.sum(dim=1) * weights[pos_mask]
        ...
        if weights is not None:
            objectness_loss = objectness_loss * weights[valid_mask]
    ...
    def forward(
        ...
        if self.training:
            ...
            weights = [1.0 if x == 'oracle' else self.cfg.LAMBDA_RPN for x in ann_types]    # for pseudo, ignore rpn loss
            weights = torch.stack([torch.full([len(p)], v) for p, v in zip(gt_labels, weights)]).to(gt_labels[0].device)
            losses = self.losses(
                anchors, pred_objectness_logits, gt_labels, pred_anchor_deltas, gt_boxes, weights
            )
        else:
            losses = {}
